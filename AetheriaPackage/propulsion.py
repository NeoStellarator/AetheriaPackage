...
def propcalc(aero, mission:AircraftParameters, engine:Engine, h_cruise:float, mesh_size=5):
    ...
    prop = Propeller(n_blades=6,
                     r_prop=np.sqrt(diskarea / np.pi),
                     rpm_cruise=900,
                     xi_0=0.1,
                     tc_ratio=0.137 # from http://airfoiltools.com/airfoil/details?airfoil=fx63137-il
                     )
    ...
    omega_max = vtip_max/prop.r_prop
    rpm_hvr = omega_max/(2*np.pi)*60 # Vertical speed of the vtol

    for T_factor in T_factors:
        #------------------- Blade geometry optimization ------------------------------------------------
        blade_cruise = BEM(data_path, prop, rho_cruise, dyn_vis, const.v_cr, n_stations=25, a=a_snd, T=T_cr_per_engine*T_factor)
        design_cruise_results = blade_cruise.optimise_blade(0)
        power_tot_cruise = design_cruise_results["pc"]*rho_cruise*(prop.rpm_cruise/60)**3*32*prop.r_prop**5 * n_prop

        #------------------- Off design analysis ---------------------------------------------------------
        n = mesh_size # Mesh size
        ...
        for col, rpm in enumerate(rpm_arr):
            hover_cond = OffDesignAnalysisBEM(data_path, prop, v_hvr, rpm, rho_cruise, dyn_vis, a_snd)
            ...
            for row, dpitch in enumerate(np.radians(pitch_arr)):
                ...
                power_req = res["power_coeff"]*rho*(rpm_hvr/60)**3*32*prop.r_prop**5

                Z_thrust[row,col] = res["thrust"]
                Z_eff[row,col] = res["eff"]*100
                Z_power[row,col] =  power_req

        if np.max(Z_thrust) > 2.25*mission.MTOM * const.g0/n_prop:
            break

    C_T_cruise = T_cr_per_engine/(rho*(prop.rpm_cruise/60)**2*(2*prop.r_prop)**4)
    prop_eff_cruise = design_cruise_results['eff']
    power_tot_hover = Z_power[Z_thrust == np.max(Z_thrust)] * n_prop

    #update to the appropriate classes
    ...
    def run_BEM(self, zeta):
        ...
        for station in stations:
            ...
            for lift_coef in Cls_trial:
                ...
                if Reyn<self.cd_interp.tree.mins[1] and Reyn > 5e6:
...
class OffDesignAnalysisBEM:
    ...
    def analyse_propeller(self, delta_pitch:float, max_iter=100, abs_extrapolation=0) -> dict:
        ...
        for station in range(len(Reyn)):
            ...
            # Maximum and minimum RN in database
            if Reyn[station]<self.cd_interp.tree.mins[1] and Reyn[station] > 5e6:
